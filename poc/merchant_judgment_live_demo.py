"""가맹점 판단 에이전트 루프의 실LLM 데모 (docs/project-overview.md 3.4.1, 3.5).

merchant_judgment_poc.py의 MockLLM 대신 LiteLLM으로 실제 Groq를 호출해
같은 트레이스가 스크립트가 아닌 실제 판단으로 재현되는지 보여준다 (판단은 Groq,
실 웹 검색만 Gemini — app/merchant_judgment.py 참고).
search_merchant 웹 검색 도구는 이번 데모에서도 mock을 유지해 트레이스를
재현 가능하게 한다 (RealSearchTool은 실 파이프라인 전용).
...
def example_delivery_app_merchant() -> None:
    """배달앱 대행 결제형 가맹점 — 검색 1회 후 확정되는 경로 (실제 Groq 호출)."""
...
def example_ambiguous_merchant() -> None:
    """사업자번호형 가맹점명 — 검색해도 특정 안 되어 리뷰 큐로 넘어가는 경로 (실제 Groq 호출)."""
...
if __name__ == "__main__":
    if not os.environ.get("GROQ_API_KEY"):
        raise SystemExit(".env에 GROQ_API_KEY를 설정하세요 (.env.example 참고)")
