...
- 커피가 주력이면서 빵/디저트도 같이 파는 카페 체인(예: 투썸플레이스, 파스쿠치, 스타벅스)은 "베이커리"가 아니라 "카페"로 분류한다. "베이커리"는 빵이 주력인 곳(예: 파리바게뜨, 성심당)에만 쓴다. 카페/베이커리 겸업처럼 보이는 체인은 검색 없이 confirm하지 말고, 먼저 decision=search로 실제 주력 품목(커피 vs 빵)을 확인한다.
- 패스트푸드·샌드위치 전문점(예: 서브웨이, 맘스터치)과 음식 배달앱(예: 배달의민족, 쿠팡이츠)은 조리된 음식을 파는 것이므로 "식료품"이 아니라 "음식점"으로 분류한다. "식료품"은 마트·식자재 배송(예: 컬리)처럼 조리 안 된 재료·상품을 파는 곳에만 쓴다.
- 가맹점명에 대학교·기관 이름이 붙어있어도(예: "~카이스트점") 그 기관 안에 있는 식당·카페일 뿐이면 실제 업종(음식점/카페 등)으로 분류한다 — 기관 이름이 보인다고 자동으로 "교육"으로 분류하지 않는다.
